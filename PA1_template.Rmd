...
The median of steps by day is **`r medianByDay`**
and the mean is **`r format(meanByDay, scientific=FALSE)`**.
...
The interval with max steps is **`r inervalWithMaxSteps$interval`**, 
the average number of steps for this interval is **`r inervalWithMaxSteps$AverageSteps`**.
...
The number of rows with missing data is **`r numberOfRowsWithMissingValues`**.
...
The median of steps by day is **`r format(medianByDayWithoutNA, scientific=FALSE)`**
and the mean is **`r format(meanByDayWithoutNA, scientific=FALSE)`**.
...
The total number of steps increased from **`r format(sum(steps$TotalSteps), scientific=FALSE)`** to **`r format(sum(stepsWithoutNA$TotalSteps), scientific=FALSE)`**.
